...
def anotaciones():
    ...
    consulta = db.execute("SELECT * FROM anotacion WHERE id_usuario = :id ",
                            id = session["id_user"])
...
def agregar():
    if request.method == "POST":
        ...
        rows = db.execute("INSERT INTO anotacion (id_usuario, titulo, contenido) VALUES (:id_usuario, :titulo, :contenido)",
...
def actualizar():
    ...
    if request.method == "POST":
        ...
        rows = db.execute("UPDATE anotacion SET titulo = :titulo, contenido = :contenido WHERE id = :id",
...
def eliminar():
    ...
    if request.method == "POST":
        ...
        rows = db.execute("DELETE FROM anotacion WHERE id = :id",
                            id = id)
        return redirect("/anotaciones")
